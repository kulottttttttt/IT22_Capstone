...
import { zoneService, type UpdateZoneDto, type Zone } from '../../services/zoneService';
...
export const EditZoneModal: React.FC<EditZoneModalProps> = ({
  ...
  useEffect(() => {
    if (zone) {
      setFormData({
        ...
        mapColorHex: zone.mapColorHex || PRESET_COLORS[0],
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      onError(error.response?.data?.message || 'Failed to update zone');
    } finally {
      setLoading(false);
    }
  };
